...
Плата по потреблению; на бесплатном тарифе сервис «засыпает» без
трафика — для магазина берите платный (Starter).
...
2. **Render → New → Web Service** → подключи репозиторий:
   ...
   - **Instance Type:** Starter (на Free сервис засыпает и оплаты
     не проверяются вовремя)
...
4. **Проверка:** открой `https://имя.onrender.com/health` — должно
   ответить `OK`, а `https://имя.onrender.com/app/` — показать магазин.
...
1. **Правильный URL.** Магазин живёт на `/app/`, а НЕ на корне:
   ...
   - `https://имя.onrender.com/health` → должно вывести `ok`
   - `https://имя.onrender.com/api/catalog` → JSON со списком сервисов
   - `https://имя.onrender.com/app/` → витрина
2. **Сервис спит (Free-план).** Первый запрос может открываться
   30–60 секунд («waking up»). Если ответ приходит после паузы —
   это сон бесплатного тарифа, апгрейд до Starter лечит.
3. **Тип сервиса.** Нужен **Web Service** (Python), а не Static Site:
   у статики нет бэкенда — `/api/*` и бот там не работают.
4. **Логи (вкладка Logs) — там ответ почти всегда. Типовые строки:**
...
5. **После успеха:** добавь `WEBAPP_URL=https://имя.onrender.com`
   (Environment) — после автопередеплоя в чате появится кнопка
   «🛍 Магазин», а deep links станут открывать нужные экраны.
6. Если `/health` отвечает `ok`, а `/app/` отдаёт 404 — папки `webapp/`
